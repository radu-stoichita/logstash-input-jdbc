...
The `schedule_allow_overlap` configuration option is true by default and allows the jdbc to execute again no matter if the previous execution has finished. If you wish to wait for the previous jdbc execution to finish before allowing a new one, set this option to false.
